...
static const char *strrnstr(const char *haystack, const char *needle,
                            size_t len) {
...
static void strSplit(const std::string &str, char delim,
...
public:
  EntryParser(const uint8_t *_buf, unsigned _len, unsigned _tiff_header_start,
              bool _alignIntel)
      : buf(_buf), len(_len), tiff_header_start(_tiff_header_start),
  ...
  std::string FetchString() const {
    return parseString(buf, length, GetData(), tiff_header_start, len,
                       alignIntel);
  }
...
public:
  ...
  static std::string parseString(const uint8_t *buf, unsigned num_components,
                                 unsigned data, unsigned base, unsigned len,
                                 bool intel) {
    std::string value;
    if (num_components <= 4) {
...
EXIFInfo::EXIFInfo(EXIFStream &stream) { parseFrom(stream); }
EXIFInfo::EXIFInfo(std::istream &stream) { parseFrom(stream); }
...
void EXIFInfo::parseIFDGPS(EntryParser &parser) {
  ...
  case 30:
    // GPS differential indicates whether differential correction is applied to
    // the GPS receiver
...
int EXIFInfo::parseFrom(std::istream &stream) {
  ...
  public:
    EXIFStdStream(std::istream &stream) : stream(stream) {
      // Would be nice to assert here that the stream was opened in binary mode,
      // but apparently that's not possible:
      // https://stackoverflow.com/a/224259/19254
    }
    bool IsValid() const override { return !!stream; }
...
int EXIFInfo::parseFromEXIFSegment(const uint8_t *buf, unsigned len) {
  ...
  const bool IS_LITTLE_ENDIAN =
      reinterpret_cast<uint8_t const *>(&_ONE32)[0] == 1;
  bool alignIntel;
  ...
  if (0x2a != EntryParser::parse16(buf + offs, alignIntel))
    return PARSE_CORRUPT_DATA;
  offs += 2;
  const unsigned first_ifd_offset =
      EntryParser::parse32(buf + offs, alignIntel);
...
int EXIFInfo::parseFromXMPSegmentXML(const char *szXML, unsigned len) {
    ...
    if (element != NULL) {
      ...
      if (szProjectionType != NULL) {
        if (0 == strcasecmp(szProjectionType, "perspective"))
          ProjectionType = 1;
        else if (0 == strcasecmp(szProjectionType, "equirectangular") ||
  ...
  if (0 == strcasecmp(Make.c_str(), "DJI") ||
      (szAbout != NULL && 0 == strcasecmp(szAbout, "DJI Meta Data"))) {
    ParseXMP::Value(document, "drone-dji:AbsoluteAltitude",
                    GeoLocation.Altitude);
    ParseXMP::Value(document, "drone-dji:RelativeAltitude",
                    GeoLocation.RelativeAltitude);
    ParseXMP::Value(document, "drone-dji:GimbalRollDegree",
                    GeoLocation.RollDegree);
    ParseXMP::Value(document, "drone-dji:GimbalPitchDegree",
                    GeoLocation.PitchDegree);
    ParseXMP::Value(document, "drone-dji:GimbalYawDegree",
    ...
    // , where Fx, Fy are focal lengths in pixels, Cx, Cy are optical center
    // offsets from the image center in pixels
  ...
  } else if (0 == strcasecmp(Make.c_str(), "senseFly") ||
             0 == strcasecmp(Make.c_str(), "Sentera")) {
  ...
  if (document->Attribute("GCamera:MicroVideo")) {
    ...
    ParseXMP::Value(document, "GCamera:MicroVideoVersion",
...
bool EXIFInfo::Geolocation_t::hasOrientation() const {
  return RollDegree != DBL_MAX && PitchDegree != DBL_MAX &&
         YawDegree != DBL_MAX;
}
