...
enum ErrorCode {
  ...
  PARSE_INVALID_JPEG =
      1, // No JPEG markers found in buffer, possibly invalid JPEG file
  PARSE_UNKNOWN_BYTEALIGN = 2, // Byte alignment specified in EXIF file was
                               // unknown (neither Motorola nor Intel)
  PARSE_ABSENT_DATA = 3,       // No EXIF and/or XMP data found in JPEG file
  PARSE_CORRUPT_DATA =
      4, // EXIF and/or XMP header was found, but data was corrupted
};
...
//
// Class responsible for storing and parsing EXIF & XMP metadata from a JPEG
// stream
//
...
public:
  ...
  EXIFInfo(std::istream
               &stream); // NB: the stream must have been opened in binary mode
  ...
  int parseFrom(std::istream &stream); // NB: the stream must have been opened
                                       // in binary mode
  ...
  // available (i.e., a blob starting with the bytes
...
public:
  ...
  uint16_t Orientation; // Image orientation, start of data corresponds to
                        ...
                        // 9: undefined
  double XResolution;   // Number of pixels per ResolutionUnit in the ImageWidth
                        // direction
  double YResolution; // Number of pixels per ResolutionUnit in the ImageLength
                      // direction
  uint16_t ResolutionUnit; // Unit of measurement for XResolution and
                           // YResolution 1: no absolute unit of measurement.
                           // Used for images that may have a non-square aspect
                           // ratio, but no meaningful absolute dimensions 2:
                           // inch 3: centimeter
  ...
  uint16_t ColorSpace; // Color Space (1=sRGB, 2=AdobeRGB, 65535=Uncalibrated)
  ...
  // 0: no red-eye reduction mode or unknown, >0: red-eye
  // reduction supported
  ...
  uint16_t WhiteBalance;   // White balance (0: auto, 1: manual)
  ...
  std::vector<uint16_t>
      SubjectArea; // Location and area of the main subject in the overall scene
                   // expressed in relation to the upper left as origin, prior
                   // to rotation 0: unknown 2: location of the main subject as
                   // coordinates (first value is the X coordinate and second is
                   // the Y coordinate) 3: area of the main subject as a circle
                   // (first value is the center X coordinate, second is the
                   // center Y coordinate, and third is the diameter) 4: area of
                   // the main subject as a rectangle (first value is the center
                   // X coordinate, second is the center Y coordinate, third is
                   // the width of the area, and fourth is the height of the
                   // area)
  struct TINYEXIF_LIB Calibration_t { // Camera calibration information
    ...
    bool hasCalibration() const; // Return true if FocalLength, OpticalCenterX,
                                 // OpticalCenterY are available (nonzero)
  } Calibration;
  struct TINYEXIF_LIB Distortion_t { // Lens distortion information
    uint32_t DewarpFlag; // Dewarp flag - indicates whether undistortion has
                         // been applied to the image UINT32_MAX: flag missing
                         // from EXIF data 0: no dewarp (raw image) - the image
                         // is distorted, should also have coefficients 1:
                         // dewarp applied (undistorted image)
    double K1;
    double K2;
    double P1;
    double P2;
    ...
    bool hasDistortion()
        const; // Return true if any of K1, K2, P1, P2, K3 are available
  } Distortion;
  struct TINYEXIF_LIB LensInfo_t { // Lens information
    ...
    double FocalPlaneXResolution;  // Number of pixels in the image width (X)
                                   // direction per FocalPlaneResolutionUnit on
                                   // the camera focal plane (may not exist)
    double FocalPlaneYResolution;  // Number of pixels in the image width (Y)
                                   // direction per FocalPlaneResolutionUnit on
                                   // the camera focal plane (may not exist)
    uint16_t
        FocalPlaneResolutionUnit; // Unit for measuring FocalPlaneXResolution
                                  // and FocalPlaneYResolution (may not exist)
                                  // 0: unspecified in EXIF data
  ...
  struct TINYEXIF_LIB Geolocation_t { // GPS information embedded in file
    ...
    uint16_t
        GPSDifferential; // Differential correction applied to the GPS receiver
    ...
    std::string GPSTimeStamp; // Time as UTC (Coordinated Universal Time) (may
                              // not exist)
    std::string GPSDateStamp; // A character string recording date and time
                              // information relative to UTC (Coordinated
                              // Universal Time) YYYY:MM:DD (may not exist)
    struct Coord_t {
      double degrees;
      double minutes;
      double seconds;
    ...
    void
    parseCoords(); // Convert Latitude/Longitude from deg/min/sec to decimal
    ...
    bool
    hasAccuracy() const; // Return true if (accuracyXY,accuracyZ) is available
  } GeoLocation;
  struct TINYEXIF_LIB
      GPano_t { // Spherical metadata.
                // https://developers.google.com/streetview/spherical-metadata
    double PosePitchDegrees; // Pitch, measured in degrees above the horizon,
                             // for the center in the image. Value must be >=
                             // -90 and <= 90.
    double PoseRollDegrees;  // Roll, measured in degrees, of the image where
                             // level with the horizon is 0. As roll increases,
                             // the horizon rotates counterclockwise in the
                             // image. Value must be > -180 and <= 180.
    bool
    hasPosePitchDegrees() const; // Return true if PosePitchDegrees is available
    bool
    hasPoseRollDegrees() const; // Return true if PoseRollDegrees is available
  } GPano;
